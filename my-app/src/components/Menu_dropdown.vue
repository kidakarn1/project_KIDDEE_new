<template>
  <div>
    <v-layout row wrap>
      <v-btn flat slot="activator" dark style="font-family: 'Kanit', sans-serif;" @click="manage_sell()">อยากขาย<v-icon>monetization_on</v-icon></v-btn>
    ...
    <v-menu offset-y v-if="status_user=='A'">
      <v-btn flat slot="activator"  style="font-family: 'Kanit', sans-serif;">จัดการ <v-icon >personal_video</v-icon></v-btn>
      <v-list>
        <v-list-tile>
    ...
    <v-menu offset-y v-if="status_user=='A'">
      <v-btn flat slot="activator"  style="font-family: 'Kanit', sans-serif;">รายงาน <v-icon >cloud_download</v-icon></v-btn>
      <v-list>
        <v-list-tile>
          <v-btn flat pl-2 class="curse"  @click="manage_report('member')" >รายงานสมาชิก</v-btn>
        </v-list-tile>
        <v-list-tile>
          <v-btn flat pl-2 class="curse"  @click="manage_report('sell')" >รายงานลงขาย</v-btn>
        </v-list-tile>
        <v-list-tile>
          <v-btn flat pl-2 class="curse"  @click="manage_report('buys')" >รายงานผู้ที่ซื้อสินค้า</v-btn>
        </v-list-tile>
        <v-list-tile>
          <v-btn flat pl-2 class="curse"  @click="manage_report('log_login')" >รายงานการใช้งานในระบบ</v-btn>
        </v-list-tile>
      </v-list>
    </v-menu>
    </v-layout>
...
import { Encode, Decode } from '@/service'
  export default {
    ...
    methods:{
      manage_report(parameters){
        if (parameters==='member'){
          console.log('45454');
          this.$router.push('/Report_member')
        }
        else if (parameters==='sell'){
          this.$router.push('/Report_sell')
        }
        else if (parameters==='buys'){
          this.$router.push('/Report_buys')
        }
        else if (parameters==='log_login'){
          this.$router.push('/Report_log_login')
        }
      },
      show_data_image(){
