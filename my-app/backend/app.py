...
def Cut_stock_buys(cursor):
    try:
        ...
        # sql_select =""" select buys_id as b_id , sell_id as sell_id , number_buy as nb_b , buys_expired_date as date_end from buys where buys_status = %s"""
        # cursor.execute(sql_select,'R')
        # columns = [column[0] for column in cursor.description]
        # result = toJson(cursor.fetchall(), columns)
