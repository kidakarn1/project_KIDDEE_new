...
@app.route('/show_chat_data', methods=['POST'])
@connect_sql()
def show_chat_data(cursor):
    try:
        data = request.json
        sql= "select * from chat where (sender = %s and recipient = %s) or (sender = %s and recipient = %s)"
        cursor.execute(sql,(data['sender'],data['recipient'],data['recipient'],data['sender']))#กับผู้ส่งกับผู้รับเพื่อให้เห็นทั้ง 2 ฝั่ง
...
@app.route('/sender_Chat', methods=['POST'])
@connect_sql()
def sender_Chat(cursor):
    try:
        data = request.json
        sql=""" insert into chat (sender, description, recipient) VALUES (%s,%s,%s)"""
        cursor.execute(sql,(data['sender'],data['description'],data['recipient']))
        return jsonify('insert_success'),200
    except Exception as e:
        return (str(e)),400
